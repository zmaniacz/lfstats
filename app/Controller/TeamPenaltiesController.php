 ...
 */
    ...
    {
        $this->Auth->allow('index', 'view', 'getPenalty', 'getTeamPenalty');
        parent::beforeFilter();
    }
    ...
    public function add($game_id, $team_color)
    {
        ...
        if ($this->request->is('post')) {
            ...
            if ($this->TeamPenalty->save($this->request->data)) {
                $this->Flash->success('The team penalty has been saved.');

                $this->TeamPenalty->Game->updateGameWinner($game_id);
                
                return $this->redirect(array('controller' => 'Games', 'action' => 'view', $game_id));
            } else {
                $this->Flash->error('The team penalty could not be saved. Please, try again.');
            }
        } else {
    ...
    public function delete($id = null)
    {
        ...
        if ($this->TeamPenalty->delete()) {
            $this->Flash->success('The team penalty has been deleted.');
        } else {
            $this->Flash->error('The team penalty could not be deleted. Please, try again.');
        }
