...
<div id="top_accordion" class="panel panel-primary">
	<div class="panel-heading" data-toggle="collapse" data-parent="#top_accordion" data-target="#collapse_standings" role="tab"
	 id="standings_heading">
	...
	<div id="collapse_standings" class="panel-collapse collapse in" role="tabpanel">
		...
		<div id="round_radio" class="btn-group" data-toggle="buttons">
			...
			<label class="btn btn-primary">
				<input type="radio" name="rounds" id="round_<?= $round['round']; ?>"
				 value="<?= $round['round']; ?>" autocomplete="off"> Round
				<?= $round['round']; ?>
			</label>
			<?php endif; ?>
			<?php endforeach; ?>
	...
	$(document).ready(function() {
		...
		var standings_table = $('#team_standings').DataTable({
			...
			"columns": [{
					data: function(row, type, val, meta) {
						if (type === 'display' && loggedIn) {
							return row.link + ' <a class="pull-right" data-toggle="modal" data-team-id="' + row.id +
					...
					data: function(row, type, val, meta) {
						if (type === 'display' && loggedIn) {
							...
							if (row.adjustment !== null) {
								result += ' <a data-toggle="modal" data-team-id="' + row.id + '" href="#viewTeamMatchPenaltyModal">(' +
									row.adjustment + ')</a>';
							}

							return result + ' <a class="pull-right" data-toggle="modal" data-team-id="' + row.id +
		...
		$('.match-select').change(function() {
			...
			$.ajax({
				url: "/leagues/ajax_assignTeam/" + $(this).data('matchId') + "/" + $(this).data('team') + "/" + $(this).val() +
					".json",
