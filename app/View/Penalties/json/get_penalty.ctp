...
<dl class="dl-horizontal">
    ...
    <dt>
        <?php echo __('Score Value'); ?>
    </dt>
    <dd>
        <?php echo h($penalty['Penalty']['value']); ?>
    </dd>
    <dt>
        <?php echo __('MVP Value'); ?>
    </dt>
    <dd>
...
<?php
    if (AuthComponent::user('role') === 'admin' || (AuthComponent::user('role') === 'center_admin' && AuthComponent::user('center') == $penalty['Scorecard']['Game']['center_id'])) {
        ...
        echo $this->Html->link("Mark Common", array('controller' => 'penalties', 'action' => 'common', $penalty['Penalty']['id']), array('class' => 'btn btn-info'));
