...
import type { CompetitionTopPlayer } from "@lfstats/db"
import {
  Table,
  TableBody,
  TableCell,
...
type SortColumn = "callsign" | "avgMvp" | "mvpPerMinute" | "totalMvp" | "avgAccuracy" | "avgHitDiff" | "winRate"
...
function getSortValue(item: CompetitionTopPlayer, col: SortColumn): string | number {
  switch (col) {
    case "callsign": return item.callsign.toLowerCase()
    case "avgMvp": return item.avgMvp
    case "mvpPerMinute": return item.mvpPerMinute
    case "totalMvp": return item.totalMvp
    case "avgAccuracy": return item.avgAccuracy
    case "avgHitDiff": return item.avgHitDiff
    case "winRate": return item.totalGames === 0 ? 0 : item.wins / item.totalGames
  }
}
    ...
    <Card>
      ...
      <CardContent className="space-y-4">
        ...
        <Table>
          <TableHeader>
            <TableRow>
              <SortableHead column="callsign" sort={sort} onSort={handleSort}>Callsign</SortableHead>
              <SortableHead column="avgMvp" sort={sort} onSort={handleSort}>Avg MVP</SortableHead>
              <SortableHead column="mvpPerMinute" sort={sort} onSort={handleSort}>MVP / min</SortableHead>
              <SortableHead column="totalMvp" sort={sort} onSort={handleSort}>Total MVP</SortableHead>
              <SortableHead column="avgAccuracy" sort={sort} onSort={handleSort}>Avg Accuracy</SortableHead>
              <SortableHead column="avgHitDiff" sort={sort} onSort={handleSort}>Avg Hit Diff</SortableHead>
              <SortableHead column="winRate" sort={sort} onSort={handleSort}>Win Rate</SortableHead>
            </TableRow>
          </TableHeader>
          <TableBody>
            {pageRows.map((p) => {
              ...
              return (
                <TableRow key={p.playerId}>
                  ...
                  <TableCell className="tabular-nums">{formatWinRate(p.wins, p.totalGames)}</TableCell>
                </TableRow>
              )
            })}
            {pageRows.length === 0 && (
