...
import type { CompetitionPositionPlayer } from "@lfstats/db";
...
function getSortValue(item: CompetitionPositionPlayer, col: SortColumn): string | number {
  switch (col) {
    case "callsign":
...
export function PositionPlayersTable({
  title,
  players,
}: {
  title: string;
  players: CompetitionPositionPlayer[];
}) {
