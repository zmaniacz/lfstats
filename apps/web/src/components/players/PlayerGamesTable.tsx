...
function enrichGame(g: PlayerGameListItem): EnrichedGame {
  ...
  const playerTeamTotal =
    (playerTeam?.score ?? 0) + (playerTeam?.eliminationBonus ?? 0)
    ...
    <DropdownMenu>
      <DropdownMenuTrigger asChild>
        <Button
          variant="outline"
          size="sm"
          className={selected.length > 0 ? "border-primary" : ""}
        >
          {triggerLabel}
...
export function PlayerGamesTable({ games }: { games: PlayerGameListItem[] }) {
  ...
  const outcomeOptions = useMemo(
    () =>
      OUTCOME_ORDER.filter((o) => enriched.some((g) => g.outcomeLabel === o)).map(
        (o) => ({ value: o, label: o }),
      ),
    [enriched],
  )
  ...
  const positionOptions = useMemo(() => {
    const positions = [...new Set(enriched.map((g) => g.position))].sort(
      (a, b) => a - b,
    )
  ...
  const filtered = useMemo(() => {
    return enriched.filter((g) => {
      if (centerFilter.length > 0 && !centerFilter.includes(g.centerId))
        return false
      if (outcomeFilter.length > 0 && !outcomeFilter.includes(g.outcomeLabel))
        return false
      if (
        positionFilter.length > 0 &&
        !positionFilter.includes(String(g.position))
      )
        return false
      return true
    })
      ...
      <Table>
        ...
        <TableBody>
          {pageGames.map((game) => {
            ...
            const positionAbbr =
              POSITIONS[game.position]?.abbr ?? String(game.position)
            ...
            return (
              <TableRow key={game.id}>
                ...
                <TableCell className={positionColor?.text ?? ""}>
                  {positionAbbr}
                </TableCell>
                <TableCell>
