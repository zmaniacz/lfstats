...
const chartConfig = {
  bottomBox: { label: "IQR", color: "hsl(var(--primary))" },
  topBox:    { label: "IQR", color: "hsl(var(--primary))" },
  ...
  return (
    <ChartContainer config={chartConfig} className="aspect-auto h-75">
      <ComposedChart data={transformed} margin={{ top: 8, right: 16, left: 16, bottom: 8 }}>
        ...
        <Bar stackId="box" dataKey="bottomBox" fill="var(--color-bottomBox)" fillOpacity={0.25} stroke="var(--color-bottomBox)" strokeWidth={1.5} />
        {/* Median line */}
        <Bar stackId="box" dataKey="bar-median" shape={<MedianBar />} />
        {/* Upper IQR box (median → Q3) */}
        <Bar stackId="box" dataKey="topBox" fill="var(--color-topBox)" fillOpacity={0.25} stroke="var(--color-topBox)" strokeWidth={1.5} />
