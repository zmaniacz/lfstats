...
function getSortValue(s: PlayerSummary, key: SortKey): string | number {
  switch (key) {
    case "callsign":       return s.callsign.toLowerCase()
    case "avgScore":       return s.avgScore
    case "avgMvp":         return s.avgMvp
    case "avgAccuracy":    return s.avgAccuracy
    case "avgHitDiff":     return s.avgHitDiff
    case "totalMedicHits": return s.totalMedicHits
    case "winRate":        return s.gameCount === 0 ? -1 : s.wins / s.gameCount
  }
}
...
function aggregate(rows: NightlyScorecardRow[]): PlayerSummary[] {
  const map = new Map<
    string,
    { entries: NightlyScorecardRow[]; playerId: string | null; iplId: string | null; callsign: string }
  >()
      ...
      <CardContent className="space-y-4">
        ...
        <div className="overflow-x-auto">
          <Table className="table-fixed min-w-175 w-full">
            <TableHeader>
              <TableRow>
                <SortableHead label="Callsign" col="callsign" className="w-[18%]" {...sortHeadProps} />
                <SortableHead label="Avg Score (Min–Max)" col="avgScore" className="w-[16%]" center {...sortHeadProps} />
                <SortableHead label="Avg MVP (Min–Max)" col="avgMvp" className="w-[16%]" center {...sortHeadProps} />
                <SortableHead label="Avg Accuracy" col="avgAccuracy" className="w-[10%]" center {...sortHeadProps} />
                <SortableHead label="Avg Hit Diff" col="avgHitDiff" className="w-[10%]" center {...sortHeadProps} />
                <SortableHead label="Total Medic Hits" col="totalMedicHits" className="w-[12%]" center {...sortHeadProps} />
                <SortableHead label="Win Rate" col="winRate" className="w-[12%]" center {...sortHeadProps} />
              </TableRow>
            </TableHeader>
            <TableBody>
              {pageRows.length === 0 ? (
                ...
                pageRows.map((s) => {
                  ...
                  <TableRow key={s.playerId ?? `guest:${s.callsign}`}>
                    ...
                    <TableCell className="text-center tabular-nums">
                      ...
                      {lt && <TrendIndicator current={s.avgScore} lifetime={lt.avgScore} label={formatScore(Math.round(lt.avgScore))} />}
                    </TableCell>
                    <TableCell className="text-center tabular-nums">
                      ...
                      {lt && <TrendIndicator current={s.avgMvp} lifetime={lt.avgMvp} label={formatMVP(lt.avgMvp)} />}
                    </TableCell>
                    <TableCell className="text-center tabular-nums">
