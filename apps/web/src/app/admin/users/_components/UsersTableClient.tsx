...
function canRevokeRole(
  ...
  const isAdminOrAbove = actorRoles.some((r) => r.role === "superAdmin" || r.role === "admin");
            ...
            <TableRow key={user.id}>
              ...
              <TableCell>
                <div className="flex flex-wrap items-center gap-2">
                  ...
                  {user.roles.map((r) => {
                    const revocable = canRevokeRole(actorRoles, r.role, r.centerId);
                    const centerLabel = r.centerShortName ?? r.centerName;
                    const revokeSummary =
                      ROLE_LABELS[r.role] + (centerLabel ? ` @ ${centerLabel}` : "");
                    return (
                      <div key={r.id} className="flex items-center gap-1">
                        <Badge variant="secondary">{ROLE_LABELS[r.role] ?? r.role}</Badge>
                        {centerLabel && <Badge variant="outline">{centerLabel}</Badge>}
                        {revocable && <RevokeRoleButton roleId={r.id} label={revokeSummary} />}
                      </div>
                    );
                  })}
