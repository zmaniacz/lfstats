...
import { getCompetitiveCompetitions, getCompetitionPenalties } from "@lfstats/db";
...
export default async function CompetitionPenaltiesPage({
  ...
  const [competitions, session] = await Promise.all([getCompetitiveCompetitions(), auth()]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between gap-4 flex-wrap">
        ...
        <CompetitionSelector
          ...
          activeParamBase="/competitions/penalties"
        />
      </div>
      ...
      <Card>
        <CardHeader>
          <CardTitle>
            {activeComp.name} · {penalties.length}{" "}
