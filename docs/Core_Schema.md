...
| `tdf_filename`       | string    | never    | The **normalized archive key** on S3 — `{country_code}-{site_code}-{YYYYMMDDHHmmss}.tdf` (e.g. `3-3-20260519201615.tdf`), built by `buildArchiveKey()` in `apps/chomper/src/ingester.ts`. This is the game slug plus `.tdf`, not the original incoming filename (which looks like `3-3_20260519201615_-_Space_Marines_5.tdf` and survives only on `chomper_job.s3_key`). Used for debugging, re-ingestion, linking to the raw file from the game view, and as the join key for the legacy migration. Forfeit games synthesized by `createForfeitGame()` store `""`. |
